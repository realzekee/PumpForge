...
export default function App() {
    ...
    const initSession = async () => {
      ...
      try {
        ...
        try {
          ...
          // Session is invalid or expired — user must log in again.
        }
        ...
        if (user) {
          ...
          const finalUserStats = {
            username: finalUsername,
            handle: profileDoc.handle || finalHandle,
            title: profileDoc.title || "Member",
            email: user.email || "",
            isPremium: profileDoc.isPremium ?? false,
            nameColor: profileDoc.nameColor || "text-zinc-400 font-bold",
            isAdmin: profileDoc.isAdmin ?? false,
          ...
          try {
             ...
             // Fetch existing wagers so "Your Position" survives a refresh
             try {
               const wDocs = await databases.listDocuments("pumpforge", "wagers", [
                 Query.equal("userId", uid),
                 Query.equal("isPaid", false),
                 Query.limit(200),
               ]);
               const betMap: Record<string, { side: "YES" | "NO"; amount: number }> = {};
               for (const w of wDocs.documents) {
                 betMap[w.polymarketId] = { side: w.choice as "YES" | "NO", amount: w.amount };
               }
               if (active) setUserBets(betMap);
             } catch (e) {
               console.warn("Could not fetch wagers:", e);
             }
          } catch(e) {
  ...
  useEffect(() => {
    ...
    // Prediction markets — fetch from Appwrite and subscribe to real-time updates
    const fetchMarkets = async () => {
      try {
        ...
        const mapped: PredictionMarket[] = res.documents.map((d: any) => ({
          id: d.$id,
          question: d.question,
          description: d.description || "",
          yesPool: d.poolYes ?? 0,
          noPool: d.poolNo ?? 0,
          yesPercentage: d.poolYes || d.poolNo
            ? Math.round(((d.poolYes ?? 0) / ((d.poolYes ?? 0) + (d.poolNo ?? 0))) * 100)
            : 50,
          userBetAmount: 0,
          userBetSide: null,
          resolved: d.status === "closed",
          resolvedOutcome: d.winningOutcome === "YES" ? "YES" : d.winningOutcome === "NO" ? "NO" : null,
    ...
    fetchMarkets();
    const marketsUnsub = client.subscribe("databases.pumpforge.collections.polymarkets.documents", () => {
      fetchMarkets();
    });
  ...
  const tradeAction = async (
    ...
    if (type === "BUY") {
      ...
      const profitDelta =
        amountCoins * (coin.price - existingHolding.avgBuyPrice);
      const nextCash = userStats.cash + totalUsdVal;
  ...
  const handlePlacePolymarketBet = async (
    ...
    try {
      ...
      // Write wager document
      await databases.createDocument("pumpforge", "wagers", ID.unique(), {
      ...
      const newPool = (side === "YES" ? market.yesPool : market.noPool) + amount;
      ...
      toast.success(`Bet placed: $${amount.toLocaleString()} on ${side}`);
    } catch (e: any) {
      console.error("Polymarket bet err", e);
      toast.error("Failed to place bet: " + (e?.message || "Unknown error"));
    }
  };
  ...
  const handleCreatePredictionLocal = async (market: any) => {
    try {
      ...
      const doc = await databases.createDocument("pumpforge", "polymarkets", ID.unique(), {
        ...
        status: "open",
        winningOutcome: null,
        endTime: market.endTime || "TBD",
        category: market.category || "general",
      });

      // Add to local state immediately (real-time sub will also fire, dedupe is fine)
      setMarkets((prev) => [
        {
          id: doc.$id,
      ...
      toast.error("Failed to create market: " + (e?.message || "Unknown error"));
    }
  };

  const handleResolvePolymarket = async (marketId: string, outcome: "YES" | "NO" | "CANCEL") => {
    try {
      const { databases } = await import("./appwrite");
      const { Query } = await import("appwrite");
      ...
      await databases.updateDocument("pumpforge", "polymarkets", marketId, {
        status: "closed",
        winningOutcome: outcome === "CANCEL" ? "REFUND" : outcome,
      });
      ...
      // 3. Get market pool totals from current state
      const market = markets.find((m) => m.id === marketId);
      const totalPool = market ? (market.yesPool || 0) + (market.noPool || 0) : 0;
      const winningPool = outcome === "YES" ? (market?.yesPool || 0) : (market?.noPool || 0);

      let payoutCount = 0;

      // 4. Process each wager
      for (const wager of wagersRes.documents) {
        ...
        if (outcome === "CANCEL") {
          ...
          // Winner: gets proportional share of the entire pool
          if (winningPool > 0) {
          ...
          // Loser — no payout, just mark paid
          await databases.updateDocument("pumpforge", "wagers", wager.$id, { isPaid: true });
          continue;
        }
      ...
      // 5. Remove from local state (market is now closed)
      setMarkets((prev) => prev.filter((m) => m.id !== marketId));

      if (outcome === "CANCEL") {
        toast.success(`Market cancelled. ${payoutCount} users refunded.`);
      } else {
        toast.success(`Market resolved ${outcome}. ${payoutCount} winners paid out!`);
      }
    } catch (error: any) {
      ...
      <main className="flex-1 min-w-0 p-5 md:p-8 max-w-7xl mx-auto flex flex-col gap-6 overflow-x-hidden">
        ...
        <Routes>
          ...
          <Route
            ...
            element={
              <PolymarketTab
                ...
                onResolveMarket={handleResolvePolymarket}
              />
            }
          />
          ...
          <Route
            ...
            element={(() => {
              const hasOwnerDashboard = !!(userStats?.isAdmin);
