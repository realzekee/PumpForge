...
import { PlusCircle, HelpCircle, AlertOctagon, Sparkles, Coins, DollarSign, Gift, CheckCircle } from 'lucide-react';
import { UserStats } from '../types';

interface CreateCoinProps {
  userStats: UserStats;
  onCreateCoin: (name: string, symbol: string, desc: string, emoji: string) => Promise<{success: boolean, error?: string}>;
}

export default function CreateCoinTab({ userStats, onCreateCoin }: CreateCoinProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!desc.trim()) {
      setErrorMsg('Please write a short pitch description to attract other traders!');
      return;
    }

    if (userStats.cash < 1100) {
      setErrorMsg('Insufficient funds. Launch costs require $1,100 ($100 list fee + $1,000 initial liquidity).');
      return;
    }

    setIsLaunching(true);
    try {
      const res = await onCreateCoin(
        name.trim(),
      ...
      <div className="lg:col-span-3 bg-zinc-900 border border-zinc-850 p-6 rounded-2xl shadow-xl">
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-850">
          <h3 className="font-extrabold text-white text-sm uppercase font-mono tracking-wider flex items-center gap-1.5">
            <PlusCircle className="text-orange-500 w-4.5 h-4.5" /> Launch custom meme coin
          </h3>
        ...
        {success ? (
          <div className="py-12 flex flex-col items-center justify-center text-center gap-3 animate-scale-up font-mono">
            ...
            <p className="text-xs text-zinc-400 max-w-sm font-medium leading-relaxed">
              Your logo has been listed on the main Market! Watch dynamic droll users in the Shill Room discuss and trade your coin.
            </p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 font-mono text-xs">
            ...
            <div className="bg-zinc-950/80 p-3.5 rounded-xl border border-zinc-950 flex flex-col gap-2 font-mono text-[11px] leading-relaxed select-none">
              ...
              <div className="flex justify-between pb-1">
                <span className="text-zinc-500">Initial TVL Liquidity injection:</span>
                <span className="text-zinc-305 text-emerald-400">$1,000.00</span>
              </div>
              <div className="h-px bg-zinc-900" />
              <div className="flex justify-between font-bold mt-1">
                <span className="text-zinc-400 font-extrabold uppercase text-[10px]">Total launching cost:</span>
            ...
            {errorMsg && (
              <div className="bg-red-500/10 border border-red-500/30 p-3 rounded-xl flex items-center gap-2 text-red-400 font-bold mb-2">
                ...
                <span className="text-[11px] leading-tight break-words">{errorMsg}</span>
              </div>
            )}
      ...
      <div className="lg:col-span-2 flex flex-col gap-4 font-mono select-none">
        ...
        <div className="bg-zinc-900 border border-zinc-805 p-5 rounded-2xl flex flex-col gap-4 text-xs font-semibold select-none leading-relaxed text-zinc-400">
          <div className="flex gap-2.5 items-start">
            <div className="w-5 h-5 rounded bg-zinc-950 flex items-center justify-center font-extrabold text-orange-400 border border-zinc-850 shrink-0 mt-0.5">1</div>
            <p>
              Inject <strong className="text-zinc-200">$1,100 total simulated cash</strong> to initiate listing. This creates your token and sets up its liquid buying pool.
            </p>
          </div>

          <div className="flex gap-2.5 items-start">
            <div className="w-5 h-5 rounded bg-zinc-950 flex items-center justify-center font-extrabold text-orange-400 border border-zinc-850 shrink-0 mt-0.5">2</div>
            <p>
              Once listed, other droll traders inside the sandbox shill-room will automatically buy/sell your coin, fluctuating its marketcap value up or down!
            </p>
          </div>
        </div>
      </div>
